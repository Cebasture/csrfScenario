...
//     const task = document.getElementById('taskInputAdmin').value.trim();
//     if (username === "" || task === "") {
//         alert("Please enter both username and task.");
//         return;
//     }
...
//         body: JSON.stringify({ username, task, assigned: 1 })
//     })
//     .then(res => res.json())
//     .then(data => {
...
function createTask() {
    ...
    // Now, make the create-task request with the CSRF token in the header
    fetch(`${API_BASE_URL}/create-task`, {
        method: "POST",
        credentials: "include",
        headers: { 
        ...
        body: JSON.stringify({ username, task })  // Removed 'assigned: 1' as the server determines it
    })
    .then(res => res.json())
    .then(data => {
        getCSRF();
...
function loadUserTasks() {
    ...
    .catch(err => {
        ...
        document.getElementById('userTaskList').innerHTML = '<li>Error loading tasks.</li>';
    });
}
...
// ----------- Load Profile Data -----------
// fetch("http://127.0.0.1:3000/adminMe", { credentials: "include" })
//     .then(res => res.json())
//     .then(user => {
//         document.getElementById('userEmail').textContent = user.email;
//     });

fetch(`${API_BASE_URL}/adminMe`, { credentials: "include" })
    .then(res => {
        if (res.status === 401) {
